...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_post_adapter_down = nn.Linear(config.n_embd, 8, bias=False)
        self.chunk_post_adapter_up = nn.Linear(8, config.n_embd, bias=False)
    ...
    def init_weights(self):
        ...
        torch.nn.init.zeros_(self.chunk_post_adapter_up.weight)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_resid.weight.numel() + self.chunk_pre_adapter_down.weight.numel() + self.chunk_pre_adapter_up.weight.numel() + self.chunk_post_adapter_down.weight.numel() + self.chunk_post_adapter_up.weight.numel() + self.chunk_decode_offsets.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_post_adapter = sum(p.numel() for p in self.chunk_post_adapter_down.parameters()) + sum(p.numel() for p in self.chunk_post_adapter_up.parameters())
        ...
        total = wte + chunk_gate + chunk_gate_hidden + chunk_gate_resid + chunk_resid + chunk_pre_adapter + chunk_post_adapter + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_gate_hidden': chunk_gate_hidden, 'chunk_gate_resid': chunk_gate_resid, 'chunk_resid': chunk_resid, 'chunk_pre_adapter': chunk_pre_adapter, 'chunk_post_adapter': chunk_post_adapter, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_post_adapter_params = list(self.chunk_post_adapter_down.parameters()) + list(self.chunk_post_adapter_up.parameters())
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_gate_params + chunk_gate_hidden_params + chunk_gate_resid_params + chunk_resid_params + chunk_pre_adapter_params + chunk_post_adapter_params + chunk_pre_norm_params + chunk_interface_params, lr=chunk_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        x = norm(x)
        x = x + self.chunk_post_adapter_up(self.chunk_post_adapter_down(x))
