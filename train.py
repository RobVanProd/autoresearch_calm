...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_phase_bias = nn.Parameter(torch.zeros(2, config.n_embd))
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_resid.weight.numel() + self.chunk_phase_bias.numel() + self.chunk_decode_offsets.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_phase_bias = self.chunk_phase_bias.numel()
        ...
        total = wte + chunk_gate + chunk_resid + chunk_phase_bias + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_resid': chunk_resid, 'chunk_phase_bias': chunk_phase_bias, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_phase_bias_params = [self.chunk_phase_bias]
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_resid_params) + len(chunk_phase_bias_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_gate_params + chunk_resid_params + chunk_phase_bias_params + chunk_pre_norm_params + chunk_interface_params, lr=chunk_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        chunk_phase = (torch.arange(chunk_T, device=x.device) & 1)
        gate = torch.sigmoid(self.chunk_gate(chunk_pair))
        x = gate * x[:, :, 0, :] + (1 - gate) * x[:, :, 1, :]
        x = x + self.chunk_phase_bias[chunk_phase][None, :, :]
