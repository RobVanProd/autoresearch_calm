...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_decode_refine_down = nn.Linear(config.n_embd, 16, bias=False)
        self.chunk_decode_refine_up = nn.Linear(16, self.chunk_size * config.n_embd, bias=False)
        self.chunk_decode_refine_gate = nn.Linear(config.n_embd, self.chunk_size * config.n_embd, bias=True)
    ...
    def init_weights(self):
        ...
        torch.nn.init.normal_(self.chunk_decode_refine_down.weight, mean=0.0, std=0.02)
        torch.nn.init.zeros_(self.chunk_decode_refine_up.weight)
        torch.nn.init.zeros_(self.chunk_decode_refine_gate.weight)
        torch.nn.init.constant_(self.chunk_decode_refine_gate.bias, -2.0)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_decode_refine_down.weight.numel() + self.chunk_decode_refine_up.weight.numel() + self.chunk_decode_refine_gate.weight.numel() + self.chunk_decode_refine_gate.bias.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_decode_refine = sum(p.numel() for p in self.chunk_decode_refine_down.parameters()) + sum(p.numel() for p in self.chunk_decode_refine_up.parameters()) + sum(p.numel() for p in self.chunk_decode_refine_gate.parameters())
        ...
        total = wte + chunk_gate + chunk_resid + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + chunk_decode_refine + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_resid': chunk_resid, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'chunk_decode_refine': chunk_decode_refine, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_decode_refine_params = list(self.chunk_decode_refine_down.parameters()) + list(self.chunk_decode_refine_up.parameters()) + list(self.chunk_decode_refine_gate.parameters())
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_resid_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_decode_refine_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_decode_params + chunk_decode_refine_params, lr=decode_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        decode_refine = self.chunk_decode_refine_up(F.silu(self.chunk_decode_refine_down(x))).view(B, chunk_T, self.chunk_size, -1)
        decode_gate = torch.sigmoid(self.chunk_decode_refine_gate(x)).view(B, chunk_T, self.chunk_size, -1)
        decode_offsets = decode_offsets + decode_gate * decode_refine
