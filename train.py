...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_decode_pos_affine = nn.Linear(config.n_embd, self.chunk_size * 2, bias=True)
    ...
    def init_weights(self):
        ...
        torch.nn.init.zeros_(self.chunk_decode_pos_affine.weight)
        torch.nn.init.zeros_(self.chunk_decode_pos_affine.bias)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_decode_pos_affine.weight.numel() + self.chunk_decode_pos_affine.bias.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_decode_pos_affine = sum(p.numel() for p in self.chunk_decode_pos_affine.parameters())
        ...
        total = wte + chunk_gate + chunk_resid + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + chunk_decode_pos_affine + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_resid': chunk_resid, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'chunk_decode_pos_affine': chunk_decode_pos_affine, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_decode_pos_affine_params = list(self.chunk_decode_pos_affine.parameters())
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_resid_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_decode_pos_affine_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_decode_params + chunk_decode_pos_affine_params, lr=decode_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        x = norm(x)
        pos_affine = self.chunk_decode_pos_affine(x).view(B, chunk_T, self.chunk_size, 2)
        pos_scale = 1.0 + 0.05 * torch.tanh(pos_affine[..., 0])
        pos_bias = 0.05 * torch.tanh(pos_affine[..., 1])

        softcap = 15
        decode_offsets = self.chunk_decode_offsets[None, None, :, :] + self.chunk_decode_proj(x).view(B, chunk_T, self.chunk_size, -1)
        decode_state = x[:, :, None, :] * pos_scale[..., None] + pos_bias[..., None]
        logits = self.lm_head(decode_state + decode_offsets)
