...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_gate_hidden = nn.Linear(self.chunk_size * config.n_embd, config.n_embd + config.n_embd // 4, bias=True)
        self.chunk_gate_resid = nn.Linear(config.n_embd + config.n_embd // 4, config.n_embd, bias=False)
        ...
        self.chunk_decode_hidden = nn.Linear(config.n_embd, config.n_embd + config.n_embd // 4, bias=True)
        self.chunk_decode_resid = nn.Linear(config.n_embd + config.n_embd // 4, config.n_embd, bias=False)
    ...
    def init_weights(self):
        ...
        torch.nn.init.normal_(self.chunk_gate_hidden.weight, mean=0.0, std=0.02)
        torch.nn.init.zeros_(self.chunk_gate_hidden.bias)
        ...
        torch.nn.init.zeros_(self.chunk_decode_resid.weight)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_decode_resid.weight.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_decode_resid = sum(p.numel() for p in self.chunk_decode_resid.parameters())
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_decode_resid_params = list(self.chunk_decode_resid.parameters())
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_gate_hidden_params) + len(chunk_gate_resid_params) + len(chunk_resid_params) + len(chunk_decode_hidden_params) + len(chunk_decode_resid_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_gate_params + chunk_gate_hidden_params + chunk_gate_resid_params + chunk_resid_params + chunk_pre_norm_params + chunk_interface_params, lr=chunk_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=chunk_decode_hidden_params + chunk_decode_resid_params, lr=chunk_lr, betas=adam_betas, eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        softcap = 15
        decode_x = x + self.chunk_decode_resid(F.silu(self.chunk_decode_hidden(x)))
        decode_offsets = self.chunk_decode_offsets[None, None, :, :] + self.chunk_decode_proj(decode_x).view(B, chunk_T, self.chunk_size, -1)
        logits = self.lm_head(decode_x[:, :, None, :] + decode_offsets)
