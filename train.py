...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_decode_refine_scale = nn.Parameter(torch.tensor(0.05))
    ...
    def init_weights(self):
        ...
        self.chunk_decode_refine_scale.fill_(0.05)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          ...
                          self.chunk_decode_refine_down.weight.numel() + self.chunk_decode_refine_up.weight.numel() +
                          self.chunk_decode_refine_scale.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_decode_refine = sum(p.numel() for p in self.chunk_decode_refine_down.parameters()) + sum(p.numel() for p in self.chunk_decode_refine_up.parameters())
        chunk_decode_refine_scale = self.chunk_decode_refine_scale.numel()
        ...
        total = wte + chunk_gate + chunk_resid + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + chunk_decode_refine + chunk_decode_refine_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_resid': chunk_resid, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'chunk_decode_refine': chunk_decode_refine, 'chunk_decode_refine_scale': chunk_decode_refine_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_decode_refine_params = list(self.chunk_decode_refine_down.parameters()) + list(self.chunk_decode_refine_up.parameters())
        chunk_decode_refine_scale_params = [self.chunk_decode_refine_scale]
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_resid_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_decode_refine_params) + len(chunk_decode_refine_scale_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_decode_refine_scale_params, lr=scalar_lr * 0.05, betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        decode_offsets = decode_offsets + self.chunk_decode_refine_scale * self.chunk_decode_refine_up(F.silu(self.chunk_decode_refine_down(x))).view(B, chunk_T, self.chunk_size, -1)
