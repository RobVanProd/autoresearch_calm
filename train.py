...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_pair_proj = nn.Linear(self.chunk_size * config.n_embd, config.n_embd, bias=True)
        self.chunk_resid = nn.Linear(config.n_embd, config.n_embd, bias=False)
        self.chunk_order_bias = nn.Parameter(torch.zeros(config.n_embd))
    ...
    def init_weights(self):
        ...
        torch.nn.init.zeros_(self.chunk_pair_proj.weight)
        torch.nn.init.zeros_(self.chunk_pair_proj.bias)
        with torch.no_grad():
            n_embd = self.config.n_embd
            self.chunk_pair_proj.weight[:, :n_embd].copy_(torch.eye(n_embd))
        torch.nn.init.zeros_(self.chunk_resid.weight)
        self.chunk_order_bias.zero_()
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          self.chunk_pair_proj.weight.numel() + self.chunk_pair_proj.bias.numel() +
                          self.chunk_resid.weight.numel() + self.chunk_order_bias.numel() + self.chunk_decode_offsets.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_pair_proj = sum(p.numel() for p in self.chunk_pair_proj.parameters())
        chunk_resid = sum(p.numel() for p in self.chunk_resid.parameters())
        chunk_order_bias = self.chunk_order_bias.numel()
        ...
        total = wte + chunk_pair_proj + chunk_resid + chunk_order_bias + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_pair_proj': chunk_pair_proj, 'chunk_resid': chunk_resid, 'chunk_order_bias': chunk_order_bias, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_pair_proj_params = list(self.chunk_pair_proj.parameters())
        chunk_resid_params = list(self.chunk_resid.parameters())
        chunk_order_bias_params = [self.chunk_order_bias]
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_pair_proj_params) + len(chunk_resid_params) + len(chunk_order_bias_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_pair_proj_params + chunk_resid_params + chunk_order_bias_params + chunk_pre_norm_params + chunk_interface_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0, chunk_specific=True),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        x0 = x[:, :, 0, :] + self.chunk_order_bias
        x1 = x[:, :, 1, :] - self.chunk_order_bias
        pair_sym = 0.5 * (x0 + x1)
        pair_diff = 0.5 * (x0 - x1)
        x = self.chunk_pair_proj(torch.cat([pair_sym, pair_diff], dim=-1))
