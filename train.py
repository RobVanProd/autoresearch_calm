...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_pair_gate = nn.Linear(self.chunk_size * config.n_embd, config.n_embd, bias=True)
    ...
    def init_weights(self):
        ...
        torch.nn.init.zeros_(self.chunk_pair_gate.weight)
        torch.nn.init.zeros_(self.chunk_pair_gate.bias)
    ...
    def estimate_flops(self):
        ...
        nparams_exclude = (self.transformer.wte.weight.numel() + value_embeds_numel +
                          self.chunk_pair_gate.weight.numel() + self.chunk_pair_gate.bias.numel() +
    ...
    def num_scaling_params(self):
        ...
        chunk_pair_gate = sum(p.numel() for p in self.chunk_pair_gate.parameters())
        ...
        total = wte + chunk_pair_gate + chunk_resid + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_pair_gate': chunk_pair_gate, 'chunk_resid': chunk_resid, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_pair_gate_params = list(self.chunk_pair_gate.parameters())
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_pair_gate_params) + len(chunk_resid_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=chunk_pair_gate_params + chunk_resid_params + chunk_pre_norm_params + chunk_interface_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0, chunk_specific=True),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        gate = torch.tanh(self.chunk_pair_gate(torch.cat([pair_sym, pair_diff], dim=-1)))
        x = pair_sym + gate * pair_diff
