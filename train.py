...
LIVE_STABILITY_VARIANT = int(os.getenv("LIVE_STABILITY_VARIANT", "2"))  # phase7-exp4: frozen compressor
...
ONLINE_FREEZE_COMPRESSOR = int(os.getenv("ONLINE_FREEZE_COMPRESSOR", "1"))  # phase7-exp4
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_post_backbone_norm = nn.LayerNorm(config.n_embd, elementwise_affine=False)
        self.chunk_post_backbone_scale = nn.Parameter(torch.zeros(config.n_embd))
        self.chunk_post_backbone_bias = nn.Parameter(torch.zeros(config.n_embd))
