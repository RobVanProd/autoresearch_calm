...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.chunk_pair_attention = nn.Linear(self.chunk_size, self.chunk_size, bias=True)
        self.chunk_pair_attention_scale = nn.Parameter(torch.tensor(0.05))
    ...
    def init_weights(self):
        ...
        torch.nn.init.eye_(self.chunk_pair_attention.weight)
        torch.nn.init.zeros_(self.chunk_pair_attention.bias)
        self.chunk_pair_attention_scale.fill_(0.05)
    ...
    def num_scaling_params(self):
        ...
        chunk_pair_attention = sum(p.numel() for p in self.chunk_pair_attention.parameters())
        chunk_pair_attention_scale = self.chunk_pair_attention_scale.numel()
        ...
        total = wte + chunk_gate + chunk_resid + chunk_pair_attention + chunk_pair_attention_scale + chunk_decode_offsets + chunk_decode_proj + chunk_pre_norm + chunk_interface + chunk_interface_scale + token_shortcut_head + shortcut_scale + value_embeds + lm_head + transformer_matrices + scalars
        return {
            'wte': wte, 'chunk_gate': chunk_gate, 'chunk_resid': chunk_resid, 'chunk_pair_attention': chunk_pair_attention, 'chunk_pair_attention_scale': chunk_pair_attention_scale, 'chunk_decode_offsets': chunk_decode_offsets, 'chunk_decode_proj': chunk_decode_proj, 'chunk_pre_norm': chunk_pre_norm, 'chunk_interface': chunk_interface, 'chunk_interface_scale': chunk_interface_scale, 'token_shortcut_head': token_shortcut_head, 'shortcut_scale': shortcut_scale,
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        chunk_pair_attention_params = list(self.chunk_pair_attention.parameters())
        chunk_pair_attention_scale_params = [self.chunk_pair_attention_scale]
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(embedding_params) +
            len(chunk_gate_params) + len(chunk_resid_params) + len(chunk_pair_attention_params) + len(chunk_pair_attention_scale_params) + len(lm_head_params) + len(token_shortcut_params) + len(chunk_decode_params) + len(chunk_pre_norm_params) + len(chunk_interface_params) + len(chunk_interface_scale_params) +
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=embedding_params + chunk_gate_params + chunk_resid_params + chunk_pair_attention_params + chunk_pre_norm_params + chunk_interface_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.0),
            dict(kind='adamw', params=chunk_pair_attention_scale_params, lr=scalar_lr * 0.05, betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0),
    ...
    def forward(self, idx, targets=None, reduction='mean'):
        ...
        pair_tokens = x.transpose(2, 3)
        pair_tokens = pair_tokens + self.chunk_pair_attention_scale * (self.chunk_pair_attention(pair_tokens) - pair_tokens)
        x = pair_tokens.transpose(2, 3)
        chunk_pair = x.reshape(B, chunk_T, self.chunk_size * x.size(-1))
